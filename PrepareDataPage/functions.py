...
def select_columns(dataframe_container, tree_frame, num_listbox, alpha_listbox):
    ...
    def keep_selected_columns():
        ...
        if selected_columns:
            ...
            messagebox.showwarning("Selection not made", "No columns selected.", parent=top)
        ...
        for column in selected_columns:
            if column in dataframe.columns:
                try:
                    ...
                    dataframe.loc[:, column] = dataframe[column].replace(old_value_converted, new_value_converted)
                except ValueError:
...
def group_numeric_data(dataframe_container, tree_frame, num_listbox, alpha_listbox):
    ...
    def apply_grouping():
        ...
        if operation == "+":
            dataframe_container['dataframe'].loc[:, new_column_name] = dataframe_container['dataframe'][col1] + dataframe_container['dataframe'][col2]
        elif operation == "-":
            dataframe_container['dataframe'].loc[:, new_column_name] = dataframe_container['dataframe'][col1] - dataframe_container['dataframe'][col2]
        elif operation == "*":
            dataframe_container['dataframe'].loc[:, new_column_name] = dataframe_container['dataframe'][col1] * dataframe_container['dataframe'][col2]
    ...
    def apply_fill(selected_columns):
        ...
        for column in selected_columns:
            if column in dataframe.columns:
                ...
                dataframe.loc[:, column] = dataframe.loc[:, column].fillna(median_value)
    ...
    def apply_fill(selected_columns):
        ...
        for column in selected_columns:
            if column in dataframe.columns:
                ...
                dataframe.loc[:, column] = dataframe[column].fillna(mean_value)
        ...
        try:
            ...
            for column in selected_columns:
                if column in dataframe_container['dataframe'].columns:
                    dataframe_container['dataframe'].loc[:, column] = dataframe_container['dataframe'][column].fillna(specific_value)
            update_treeview(dataframe_container['dataframe'], tree_frame)
    ...
    def apply_mice(selected_columns):
        ...
        try:
            ...
            for col in selected_columns:
                dataframe.loc[:, col] = imputed_df[col]
            dataframe_container['dataframe'] = dataframe
...
def standardize_numeric(dataframe_container, tree_frame, columns):
    ...
    def apply_standardization(selected_columns):
        ...
        try:
            ...
            dataframe.loc[:, selected_columns] = scaler.fit_transform(dataframe.loc[:, selected_columns])
            dataframe_container['dataframe'] = dataframe
            update_treeview(dataframe_container['dataframe'], tree_frame)
...
def normalize_numeric(dataframe_container, tree_frame, columns):
    ...
    def apply_normalization(selected_columns):
        ...
        try:
            scaler = MinMaxScaler()
            dataframe.loc[:, selected_columns] = scaler.fit_transform(dataframe[selected_columns])
            dataframe_container['dataframe'] = dataframe
            update_treeview(dataframe_container['dataframe'], tree_frame)
        ...
        try:
            ...
            for column in selected_columns:
                if method == 'percentile':
                    ...
                    raise ValueError("Unsupported method provided for outlier detection")

                if action == 'remove':
                    dataframe = dataframe[~outlier_mask].reset_index(drop=True)
                elif action == 'nan':
                    dataframe.loc[outlier_mask, column] = float('nan')
                else:
                    raise ValueError("Unsupported action provided for handling outliers")
...
def string_slicing_alphanumeric(dataframe_container, tree_frame, columns):
    top = tk.Toplevel()
    ...
    def apply_slicing():
        ...
        try:
            for column in selected_columns:
                if method == 'range':
                    dataframe_container['dataframe'].loc[:, column] = dataframe_container['dataframe'][column].str[int(start):int(end)]
                elif method == 'from_end':
                    dataframe_container['dataframe'].loc[:, column] = dataframe_container['dataframe'][column].str[:-int(start)]
                elif method == 'remove_substring':
                    dataframe_container['dataframe'].loc[:, column] = dataframe_container['dataframe'][column].str.replace(substring, '', regex=False)
...
def binary_encoding_alphanumeric(dataframe_container, tree_frame, columns, num_listbox, alpha_listbox):
    top = tk.Toplevel()
...
def numeric_encoding_alphanumeric(dataframe_container, tree_frame, columns, num_listbox, alpha_listbox):
    top = tk.Toplevel()
